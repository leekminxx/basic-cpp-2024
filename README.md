...
- Name Space
	...
	>> 장점 : 이름 충돌 방지 , 코드의 구조화 및 조직화 
		>>> 이름 충돌 방지: 예를 들어, 두 개의 다른 라이브러리에서 같은 이름의 함수를 정의할 수 있음

	>> 단점 : 복잡성 증가 , 긴 이름 사용
		>>> 복잡성 증가 : 네임스페이스를 과도하게 사용하면 코드가 복잡해질 수 있음 너무 많은 네임스페이스를 사용하면 관리가 어려워질 수 있음
		>>> 긴 이름 사용 : 사용할 때 마다 긴 이름을 작성해야 하는 경우가 있음 이 점은 코드의 가독성을 떨어뜨릴수 있음
	```c
